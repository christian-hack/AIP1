...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            # store coordinate of current position
            x,y = state[0]
            dx, dy = Actions.directionToVector(action)
            # store successor node coordinates
            sucx, sucy = int(x + dx), int(y + dy)
            # check for bumping into wall
            isWall = self.walls[sucx][sucy]
            # if wall is not being hit
            if isWall is False:
                nv = []
                oldNV = state[1]
                # if position is new corner, append to nv (not visited) list
                for position in oldNV:
